...
const TodoTask = (props: ITodo) => {
  ...
  return (
    <View style={tw`flex flex-row justify-between m-1 border border-green-500  ${style}`}>
      <Text style={tw`self-center p-2 bg-yellow-50 `} onPress={() => onPress(id)}>
        {title}
      </Text>
      <Button text="Delete" onPress={() => deleteTask(id)} style="bg-red-500 w-20" textStyle="text-white" />
    </View>
  );
};
...
export default function App() {
  ...
  const addTask = () => {
    ...
    const newTask: TodoType = { id: tasks.length, title: task, completed: false };
  ...
  return (
    <View style={tw`flex-1 bg-yellow-100 justify-center content-center`}>
      ...
      {tasks.map((task, index) => (
        <TodoTask key={index} task={task} onPress={toggleTask} onLongPress={deleteTask} />
      ))}
